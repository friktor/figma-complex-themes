...
const mapSelections = (selections: readonly SceneNode[]): SelectionEvent[] =>
  selections.map(({ name, type, id }) => ({ name, type, id }))

export const getCurrentSelections = async (): Promise<SelectionEvent[]> => mapSelections(figma.currentPage.selection)

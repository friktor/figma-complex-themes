...
export const convertGradientStops = {
    fromFigma: (gradientStops: ColorStop[]): Stop[] => gradientStops.map(({ position, color: _c }) => ({
        offset: position.toString(),
    
        color: (
            (_rgba) => `rgba(${_rgba.join(",")})`)
            (chroma(_c.r, _c.g, _c.b, _c.a, 'gl').rgba(),
        ),
    })),

    toFigma: (gradientStops: Stop[]): ColorStop[] => gradientStops.map(({ offset, color }) => ({
...
export const convertGradientAngle = {
    toMatrix: (angleDeg: number): Transform => ((angle) => ([
    ...
    fromMatrix: (gradientTransform: Transform): number => (
        Math.asin(gradientTransform[0][1]) * (180 / Math.PI) + 90
    ),
}
...
export const createSolidPaint = (color: string = "#000"): SolidPaint => {
...
export const createGradientLinear = (angleDeg?: number, stops?: Stop[]): GradientPaint => {
    const _stops = [
        createGradientStop.forFigma(0, "#000"),
        createGradientStop.forFigma(1, "#FFF"),
    ]
